...
export default function DashboardPage() {
    ...
    const [orders, setOrders] = useState<{ id: string; orderNumber: string; total: number; status: string; createdAt: string }[]>([])
    const [subscriptions, setSubs] = useState<{ id: string; status: string; frequency: string; quantity: number }[]>([])
    const [referralStats, setReferralStats] = useState<{ totalEarnedLitres: string; successfulReferrals: number; pendingReferrals: number; referralCode: string } | null>(null)
    const [loyaltyPoints, setLoyaltyPoints] = useState<{ balance: number; totalEarned: number } | null>(null)

    // Auth redirect handled by dashboard/layout.tsx — no redirect needed here

    useEffect(() => {
        if (!user) return
        authFetch('/api/wallet').then(d => d.success && setWallet(d.data))
        authFetch('/api/orders').then(d => d.success && setOrders(d.data?.slice(0, 5) || []))
        authFetch('/api/subscriptions').then(d => d.success && setSubs(d.data || []))
        if (user.role === 'B2C') {
            authFetch('/api/referral').then(d => d.success && setReferralStats(d.data))
            authFetch('/api/loyalty').then(d => d.success && setLoyaltyPoints(d.data))
        }
    }, [user, authFetch])
            ...
            <div className="dashboard-layout">
                <aside className="dashboard-sidebar">
                    ...
                    <ul className="sidebar-menu">
                        ...
                        {user.role === 'B2C' && <li><Link href="/dashboard/referrals">⭐ Referrals</Link></li>}
                    </ul>
                </aside>
                    ...
                    <div className="stats-row">
                        ...
                        {loyaltyPoints && (
                            <div className="stat-card" style={{ background: 'linear-gradient(135deg, #F0F7FF 0%, #FFF9ED 100%)', border: '1px solid #E0E7FF' }}>
                                ...
                                <div className="stat-value">{loyaltyPoints.balance}</div>
                                <div style={{ fontSize: '12px', color: 'var(--gray-500)', marginTop: '2px' }}>= ₹{loyaltyPoints.balance} value</div>
                        ...
                        <div className="card" style={{ padding: '20px', marginBottom: '24px', background: 'linear-gradient(135deg, #FFF7ED 0%, #FFFBEB 100%)', border: '1px solid #FDE68A' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <div>
                                    ...
                                    <div style={{ fontSize: '22px', fontWeight: 800, color: 'var(--navy-800)' }}>{referralStats.totalEarnedLitres} earned</div>
                                    <div style={{ fontSize: '13px', color: 'var(--gray-500)', marginTop: '2px' }}>{referralStats.successfulReferrals} rewarded · {referralStats.pendingReferrals} pending</div>
                                </div>
